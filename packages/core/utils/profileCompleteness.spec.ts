...
describe("computeProfileCompleteness", () => {
  ...
  it("drops 15% when only one of metro/shift preferences is set", () => {
    expect(
      computeProfileCompleteness({ ...baseProfile, preferredShiftTimes: [] })
        .percent,
    ).toEqual(85);
  });
  ...
  it("drops 10% when portfolio is empty", () => {
    expect(
      computeProfileCompleteness({ ...baseProfile, portfolioPhotos: [] })
        .percent,
    ).toEqual(90);
  });
  ...
  it("still counts hourlyRate when only one bound is set", () => {
    expect(
      computeProfileCompleteness({ ...baseProfile, hourlyRateMax: undefined })
        .percent,
    ).toEqual(100);
  });
  ...
  it("drops 10% when work history is empty", () => {
    expect(
      computeProfileCompleteness({ ...baseProfile, workExperiences: [] })
        .percent,
    ).toEqual(90);
  });
  ...
  it("drops 10% when work history is undefined", () => {
    expect(
      computeProfileCompleteness({ ...baseProfile, workExperiences: undefined })
        .percent,
    ).toEqual(90);
  });
  ...
  it("drops 10% when medical book is expired", () => {
    expect(
      computeProfileCompleteness({
        ...baseProfile,
        medicalBookExpiresOn: "2000-01-01",
      }).percent,
    ).toEqual(90);
  });
