...
(c) где хранится Ваша Персональная информация и кому она может быть передана;
(d) какие инструменты Вы можете использовать для сокращения объёма собираемой Оператором Персональной информации о Вас;
(e) каковы Ваши права в рамках проводимой обработки Персональной информации.
...
Вы не обязаны предоставлять Оператору Персональную информацию, за исключением случаев, предусмотренных Условиями использования Сервиса или условиями отдельных функций. Тем не менее, Вы обязуетесь предоставлять достоверную и достаточную Персональную информацию, а также своевременно обновлять её.

Оператор может собирать следующие категории Персональной информации о Вас во время использования Вами Сервиса:

(i) Персональная информация, предоставленная Вами при регистрации (создании учётной записи), такая как Ваше имя, адрес электронной почты, выбранная роль (бариста / бизнес), пароль (хранится в виде криптографического хэша);
...
(iv) приблизительные данные о геолокации (только во время использования Сервиса и только при наличии Вашего отдельного разрешения, предоставленного через системные настройки устройства или браузера);
(v) технические данные: идентификатор устройства, версия операционной системы и браузера, журналы сбоев, push-токен или подписка на браузерные push-уведомления, дата и время доступа к Сервису, IP-адрес, cookie сессии Веб-версии (см. раздел 10);
...
(iv) для конкретных целей либо в соответствии с требованием применимого законодательства Оператор может запросить Ваше отдельное согласие на обработку Персональной информации, в том числе посредством отметки на экране регистрации в Приложении или Веб-версии.
...
(iii) осуществление связи с Вами для направления Вам уведомлений, запросов и информации, относящейся к работе Сервиса (например, код подтверждения и восстановление пароля по электронной почте, push-уведомления о новом сообщении, отклике на вакансию, изменении статуса смены); рекламные уведомления не отправляются;
...
6. ГДЕ ХРАНИТСЯ И КАК ЗАЩИЩАЕТСЯ ВАША ПЕРСОНАЛЬНАЯ ИНФОРМАЦИЯ

6.1. Место хранения

База данных Сервиса (учётные записи, профили, вакансии, отклики, переписка), загруженные Вами файлы (фотографии) и резервные копии размещаются на серверах, находящихся на территории Российской Федерации: в центрах обработки данных в Москве и Санкт-Петербурге на инфраструктуре АО «Таймвэб» (Timeweb Cloud). Резервные копии создаются ежедневно и хранятся не более 14 дней.

6.2. Меры защиты

В большинстве случаев Персональная информация обрабатывается автоматически без доступа к ней кого-либо из лиц, действующих на стороне Оператора. В случае если такой доступ потребуется, он будет предоставлен только тем лицам, которые нуждаются в этом для выполнения своих задач (например, модератору для рассмотрения жалобы). Для защиты и обеспечения конфиденциальности Персональной информации все указанные лица обязаны соблюдать внутренние правила и процедуры в отношении обработки Персональной информации.
...
В частности, Персональная информация передаётся между Вашим устройством и серверами Оператора по защищённому каналу связи (TLS); доступ к серверам ограничен ключами доступа; разграничение доступа к данным в базе выполняется на уровне строк для каждой учётной записи. Пароли хранятся исключительно в виде криптографических хэшей и не доступны Оператору в исходном виде.
...
(i) поставщик хостинга и облачной инфраструктуры — АО «Таймвэб» (Timeweb Cloud), Россия: серверы, на которых размещены база данных, файлы и резервные копии Сервиса;
(ii) провайдеры входа — Apple Inc. (США), Google LLC (США), ООО «ЯНДЕКС» (Россия) — при использовании Вами соответствующего способа авторизации;
(iii) службы доставки push-уведомлений — Apple Push Notification Service (Apple Inc., США) для Приложения; push-службы браузеров Google LLC, Apple Inc., Mozilla Corporation (США) для Веб-версии. Содержимое браузерных уведомлений шифруется на стороне Оператора и недоступно поставщику push-службы;
(iv) сервис доставки электронной почты — Resend Inc. (США): получает Ваш адрес электронной почты и текст служебного письма (код подтверждения, восстановление пароля);
(v) хостинг Веб-версии и сайта bystrobarista.com — Vercel Inc. (США): при использовании Веб-версии Ваши запросы (IP-адрес, cookie сессии, данные отображаемых страниц) обрабатываются серверами Vercel транзитно, без сохранения в базах данных Vercel; сервис Vercel Analytics собирает обезличенную статистику посещений без использования cookie;
(vi) картографический сервис — ООО «ЯНДЕКС» (Россия): адреса заведений и координаты для отображения на карте;
(vii) лица, участвующие в выявлении угроз безопасности для Сервиса, пользователей, Оператора и/или третьих лиц.
...
7.3. Локализация и трансграничная передача

Оператор осуществляет запись, систематизацию, накопление, хранение, уточнение (обновление, изменение), извлечение Персональной информации граждан Российской Федерации с использованием баз данных, находящихся на территории Российской Федерации (раздел 6.1), как этого требует Федеральный закон № 152-ФЗ.

Часть поставщиков услуг, указанных в разделе 7.1 (Apple Inc., Google LLC, Mozilla Corporation, Resend Inc., Vercel Inc.), находится за пределами территории Российской Федерации (США). Передача им Персональной информации является трансграничной передачей, осуществляется только после выполнения формальностей, предусмотренных статьёй 12 Федерального закона № 152-ФЗ (уведомление Роскомнадзора), и только в объёме, необходимом для соответствующей функции: авторизация через Apple или Google, доставка уведомлений и служебных писем, работа Веб-версии. Основная база данных Сервиса за пределы Российской Федерации не передаётся.
...
10. COOKIE, ИДЕНТИФИКАТОРЫ И ХРАНИЛИЩА УСТРОЙСТВА

10.1. Приложение

Приложение не использует веб-cookie. Однако оно использует ряд технологий, выполняющих аналогичные функции:
...
10.2. Веб-версия (app.bystrobarista.com)

Веб-версия использует только строго необходимые cookie и хранилище браузера, без которых вход в учётную запись и работа Сервиса невозможны:
(i) cookie сессии «sb-bystrobarista-auth-token» — токены Вашего входа в учётную запись; хранится до выхода из учётной записи или истечения срока действия сессии;
(ii) cookie «bb_profile» — роль и статус Вашего профиля для маршрутизации страниц; срок хранения 5 минут;
(iii) cookie «bb_lang» — выбранный язык интерфейса; срок хранения 1 год;
(iv) локальное хранилище браузера — настройки интерфейса, черновики и данные подписки на браузерные push-уведомления (только если Вы разрешили уведомления в браузере).
Cookie третьих лиц для рекламы или отслеживания в Веб-версии не используются. Сервис Vercel Analytics и Speed Insights собирает обезличенную статистику посещений (страница, страна, тип устройства) без установки cookie и без идентификации Пользователя.

10.3. Сайт bystrobarista.com
...
Токены сессии хранятся до выхода из учётной записи или до их истечения. Push-токен хранится до удаления Приложения с устройства или отзыва разрешения на уведомления в браузере. Локальные данные хранятся до удаления Приложения либо до очистки данных сайта в браузере. Push-токены передаются Apple Push Notification Service и push-службам браузеров для доставки уведомлений. Информация о сбоях может передаваться в обезличенном виде Оператору для диагностики проблем. Доступ третьих лиц к содержимому защищённого хранилища устройства ограничен средствами операционной системы.

Вы можете в любой момент выйти из учётной записи в разделе Настройки, что приведёт к удалению токенов сессии с устройства или из браузера. Удаление Приложения с устройства приводит к удалению всех локально хранимых данных.
...
Оператор обязуется не вносить существенных изменений, не налагать дополнительных обременений или ограничений Ваших прав, установленных настоящей Политикой, без Вашего уведомления. Вы будете уведомлены о таких изменениях. Соответствующие уведомления могут быть отображены в Сервисе (например, через экран подтверждения обновлённых документов при входе) до момента, когда изменения вступят в силу, или могут быть отправлены Вам по другим каналам связи (например, по электронной почте, если Вы предоставили её Оператору).

Редакция от 15 сентября 2026 г. отражает перенос базы данных Сервиса на серверы в Российской Федерации и уточняет перечень поставщиков услуг и порядок использования cookie в Веб-версии.
...
(c) where Your Personal Information is stored and to whom it may be transferred;
(d) what tools You can use to reduce the amount of Personal Information the Operator collects about You;
(e) Your rights in connection with the processing of Personal Information.
...
You are not obligated to provide the Operator with Personal Information, except in cases provided for by the Service's Terms of Service or the terms of individual features. However, You undertake to provide accurate and sufficient Personal Information and to update it in a timely manner.

The Operator may collect the following categories of Personal Information about You while You use the Service:

(i) Personal Information provided by You upon registration (creation of an account), such as Your name, email address, selected role (barista / business), and password (stored as a cryptographic hash);
...
(iv) approximate location data (only while the Service is in use and only with Your separate permission given through the device or browser settings);
(v) technical data: device identifier, OS and browser version, crash logs, push token or browser push subscription, date and time of access to the Service, IP address, Web Version session cookie (see section 10);
...
(i) processing is necessary to perform agreements between You and the Operator, such as the Service's Terms of Service, which also includes ensuring the operation of the Service (for example, surfacing relevant vacancies, delivering messages between You and other users);
...
(iv) for specific purposes, or as required by applicable law, the Operator may request Your separate consent to process Personal Information, including via a checkbox on the sign-up screen in the App or the Web Version.
...
(iii) communicating with You to send notifications, requests, and information related to the operation of the Service (for example, confirmation code and password recovery by e-mail, push notifications about a new message, job application, shift status change); promotional notifications are not sent;
...
6. WHERE YOUR PERSONAL INFORMATION IS STORED AND HOW IT IS PROTECTED

6.1. Storage location

The Service database (accounts, profiles, vacancies, applications, messages), the files You upload (photos) and backups are hosted on servers located in the Russian Federation: in data centres in Moscow and Saint Petersburg on the infrastructure of JSC "Timeweb" (Timeweb Cloud). Backups are made daily and kept for no more than 14 days.

6.2. Security measures

In most cases, Personal Information is processed automatically without access by any person acting on the Operator's behalf. Where such access is required, it is granted only to those persons who need it to perform their duties (for example, a moderator reviewing a complaint). To protect Personal Information and maintain its confidentiality, all such persons are required to comply with internal rules and procedures regarding the processing of Personal Information.
...
In particular, Personal Information is transmitted between Your device and the Operator's servers over a secure channel (TLS); server access is restricted to key-based authentication; access to data in the database is separated per account at the row level. Passwords are stored only as cryptographic hashes and are not available to the Operator in plaintext.
...
(i) hosting and cloud infrastructure provider — JSC "Timeweb" (Timeweb Cloud), Russia: the servers on which the Service database, files and backups are hosted;
(ii) sign-in providers — Apple Inc. (USA), Google LLC (USA), Yandex LLC (Russia) — when You use the corresponding sign-in method;
(iii) push delivery services — Apple Push Notification Service (Apple Inc., USA) for the App; browser push services of Google LLC, Apple Inc., Mozilla Corporation (USA) for the Web Version. The content of browser notifications is encrypted on the Operator's side and is not accessible to the push service provider;
(iv) e-mail delivery service — Resend Inc. (USA): receives Your email address and the text of the service e-mail (confirmation code, password recovery);
(v) hosting of the Web Version and the website bystrobarista.com — Vercel Inc. (USA): when You use the Web Version, Your requests (IP address, session cookie, data of the rendered pages) are processed by Vercel servers in transit, without being stored in Vercel databases; Vercel Analytics collects anonymized visit statistics without cookies;
(vi) map service — Yandex LLC (Russia): venue addresses and coordinates for displaying the map;
(vii) persons assisting in the detection of security threats to the Service, users, the Operator, and/or third parties.
...
(iii) any third party for the purpose of ensuring the legal protection of the Operator or third parties in the event of Your violation of the Service's Terms of Service, this Policy, or the Consent to Personal Data Processing, or where a threat of such violation exists.

The Operator does not sell Your Personal Information and does not transfer it for advertising purposes.

7.3. Localization and cross-border transfer

The Operator carries out the recording, systematization, accumulation, storage, clarification (updating, modification), and retrieval of Personal Information of citizens of the Russian Federation using databases located on the territory of the Russian Federation (section 6.1), as required by Federal Law No. 152-FZ.

Some of the service providers listed in section 7.1 (Apple Inc., Google LLC, Mozilla Corporation, Resend Inc., Vercel Inc.) are located outside the territory of the Russian Federation (USA). Transfer of Personal Information to them is a cross-border transfer, carried out only after the formalities required by Article 12 of Federal Law No. 152-FZ (notification of Roskomnadzor) have been completed, and only to the extent necessary for the corresponding function: sign-in via Apple or Google, delivery of notifications and service e-mails, operation of the Web Version. The primary Service database is not transferred outside the Russian Federation.
...
If You want any of Your Personal Information to be deleted from the Operator's databases, You can delete it Yourself through the Service interface (where applicable) or delete Your account in Settings → Delete account. After deleting the account, Personal Information is removed within 30 (thirty) days, except for data that the Operator is legally required to retain (for example, for dispute resolution or tax accounting). Data is removed from backups when their retention period expires (no more than 14 days).
...
10. COOKIES, DEVICE IDENTIFIERS AND STORAGE

10.1. The App

The App does not use web cookies. However, it uses a number of technologies that serve a similar purpose:
...
10.2. The Web Version (app.bystrobarista.com)

The Web Version uses only strictly necessary cookies and browser storage, without which signing in and using the Service is impossible:
(i) the session cookie "sb-bystrobarista-auth-token" — Your sign-in tokens; stored until You sign out or the session expires;
(ii) the "bb_profile" cookie — Your profile role and status used for page routing; stored for 5 minutes;
(iii) the "bb_lang" cookie — the selected interface language; stored for 1 year;
(iv) browser local storage — interface preferences, drafts and the browser push subscription (only if You allowed notifications in the browser).
No third-party advertising or tracking cookies are used in the Web Version. Vercel Analytics and Speed Insights collect anonymized visit statistics (page, country, device type) without setting cookies and without identifying the User.

10.3. The website bystrobarista.com
...
Session tokens are stored until You sign out of Your account or until they expire. The push token is stored until the App is removed from the device or notification permission is revoked in the browser. Local data is stored until the App is removed, or until site data is cleared in the browser. Push tokens are transmitted to Apple Push Notification Service and to browser push services for the delivery of notifications. Crash information may be sent to the Operator in anonymized form for diagnostic purposes. Third-party access to the contents of the device's secure storage is restricted by the operating system.

You can sign out of Your account in the Settings section at any time, which will remove session tokens from the device or the browser. Removing the App from the device deletes all locally stored data.
...
The Operator undertakes not to make material changes, nor to impose additional burdens or restrictions on Your rights as set out in this Policy, without notifying You. You will be notified of such changes. The corresponding notifications may be displayed in the Service (for example, via the updated-documents confirmation screen at sign-in) before the changes take effect, or may be sent to You through other communication channels (for example, by email, if You have provided one to the Operator).

The edition of 15 September 2026 reflects the move of the Service database to servers in the Russian Federation and clarifies the list of service providers and the use of cookies in the Web Version.
